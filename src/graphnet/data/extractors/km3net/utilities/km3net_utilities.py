...
def create_unique_id_filetype(
    ...
    Args:
        pdg_id (List[int]): List of particle ID codes (PDG codes).
        energy (List[float]): List of particle energies in GeV.
        is_cc_flag (List[int]): List of flags indicating the interaction type.
            1 for charged current (CC), 0 for neutral current (NC).
        ...
        model_hnl (str): Model name for HNL files.
    ...
    for i in range(len(pdg_id)):
            ...
            if pdg_id[i] in [12, -12, 14, -14, 16, -16]:
                if is_cc_flag[i] == 1:
                    particle = (
                        "elec_"
                        if abs(pdg_id[i]) == 12
                        else "muon_" if abs(pdg_id[i]) == 14 else "tau_"
                    )
                elif is_cc_flag[i] == 0:
                    particle = ""
                else:
                ...
                nc = (
                    "NC_"
                    if is_cc_flag[i] == 0 and abs(pdg_id[i]) == 14
                    else ""
                )
                if energy[i] <= 100:
                    energy_range = "1_100"
                    ...
                    raise ValueError(
                        "The energy is not in the expected range [1, 10000]."
                    )
                file_id = str(code_dict[f"{anti}{nc}{particle}{energy_range}"])
        elif file_type == "muon":
            file_id = str(code_dict["atm_muon"])
        elif file_type == "noise":
            file_id = str(code_dict["noise"])
        elif file_type == "data":
            file_id = str(code_dict["data"])
        elif file_type == "hnl":
        ...
        unique_id.append(
            str(evt_id[i]) + "000" + str(run_id[i]) + "0" + str(file_id)
        )
